      ...
      {product.map((val) => {
        ...
        return (
          <div
            ...
            key={_id}
          >
            <div className="basis-[20%] max-lg:basis-[40%] max-sm:basis-[100%]">
              <Link href={`products/${_id}`}>
                <Image
                  src={thumbnail}
                  width={300}
                  height={300}
                  className="flex h-[100%] items-stretch  max-sm:w-[100%] max-sm:h-[280px] object-fill rounded-tl-lg rounded-bl-lg"
                  alt={title}
                />
              </Link>
            </div>
            <div className="basis-[60%] max-sm:px-2 max-sm:basis-[100%] max-sm:border-none max-lg:basis-[70%] py-4 border-r-[1px] border-[#e0dede]">
              <Link href={`products/${_id}`}>
